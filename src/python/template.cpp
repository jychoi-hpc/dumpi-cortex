...
extern const dumpi_time* cortex_python_current_perf;

static void %%dumpi_operation%%_to_python(bp::dict& args, const %%dumpi_operation%% *prm) {
%%convertor%%
}

extern "C" int cortex_python_translate_%%MPI_Operation%%(const %%dumpi_operation%% *prm, 
			uint16_t thread, 
	...
	bp::object fun;
	try {
		fun = user_python_translator.attr("%%MPI_Operation%%");
	} catch(const bp::error_already_set&) {
		// No translation provided, just forward the call
		PyErr_Clear();
		cortex_dumpi_profile* profile = (cortex_dumpi_profile*)uarg;
		cortex_post(profile, DU%%MPI_Operation%%, prm, thread, cpu, wall, perf);
		return 0;
	}
	...
	try {
		bp::dict arguments;
		%%dumpi_operation%%_to_python(arguments,prm);

...
static void dumpi_func_call_to_python(bp::dict& args, const dumpi_func_call *prm) {

    args["fn"]	= prm->fn;

}

extern "C" int cortex_python_translate_MPI_Func_call(const dumpi_func_call *prm, 
			uint16_t thread, 
	...
	bp::object fun;
	try {
		fun = user_python_translator.attr("MPI_Func_call");
	} catch(const bp::error_already_set&) {
		// No translation provided, just forward the call
		PyErr_Clear();
		cortex_dumpi_profile* profile = (cortex_dumpi_profile*)uarg;
		cortex_post(profile, DUMPI_Func_call, prm, thread, cpu, wall, perf);
		return 0;
	}
	...
	try {
		bp::dict arguments;
		dumpi_func_call_to_python(arguments,prm);
